...
const ArchivedEpcFilter: React.FC<ArchivedEpcFilterProps> = ({ stockFlow }) => {
	const { t } = useTranslation()
	const [filterOpen, setFilterOpen] = useState<boolean>(false)
	const { data } = useGetDeletedEpcSpecsQuery()
	...
	const [persistentFormValues, setPersistentFormValues] = usePersistentFilterState(stockFlow)
				...
				<Form {...form}>
					<FilterForm className='space-y-1.5' onSubmit={form.handleSubmit(handleSearch)}>
						...
						<Div as='fieldset' className='space-y-6'>
							...
							<FormField
								name={stockFlow === StockFlow.INBOUND ? 'scannable' : 'scanned'}
