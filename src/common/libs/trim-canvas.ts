...
function getRGBA(x: number, y: number, imgWidth: number, imgData: ImageDataArray) {
	return {
...
function getAlpha(x: number, y: number, imgWidth: number, imgData: ImageDataArray) {
	return getRGBA(x, y, imgWidth, imgData).alpha
}

// finds the first y coord in imgData that is not white
function scanY(fromTop: boolean, imgWidth: number, imgHeight: number, imgData: ImageDataArray) {
...
function scanX(fromLeft: boolean, imgWidth: number, imgHeight: number, imgData: ImageDataArray) {
