		...
		<SidebarContext.Provider value={contextValue}>
			<TooltipProvider delayDuration={0}>
				<div
					...
					className={cn(
						'group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar z-50 flex min-h-svh w-full',
						className
					)}
					{...props}>
					{children}
...
> = ({ side = 'left', variant = 'sidebar', collapsible = 'offcanvas', className, children, ...props }) => {
	...
	if (collapsible === 'none') {
		return (
			<div
				className={cn(
					'bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width) scrollbar-none! flex-col overflow-x-auto',
					className
				)}
				{...props}>
				{children}
	...
	return (
		<div
			className='group peer text-sidebar-foreground block sm:hidden md:hidden'
			data-state={state}
			...
			<div
				className={cn(
					'fixed inset-y-0 flex h-svh w-(--sidebar-width) transition-[left,right,width] duration-100 ease-linear sm:hidden md:hidden',
					side === 'left'
...
const SidebarTrigger: React.FC<React.ComponentProps<'button'>> = ({ className, onClick, ...props }) => {
...
const SidebarRail: React.FC<React.ComponentProps<'button'>> = ({ className, ...props }) => {
	...
	return (
		<button
			...
			className={cn(
				'bg-sidebar after:bg-sidebar-border hover:after:bg-sidebar-border/50 absolute inset-y-0 z-30 flex w-4 -translate-x-1/2 transition-opacity ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:h-full after:w-px after:transition-all after:duration-200 sm:hidden md:hidden',
...
const SidebarSeparator: React.FC<React.ComponentProps<typeof Separator>> = ({ className, ...props }) => {
	return <Separator data-sidebar='separator' className={cn('bg-sidebar-border mx-2 w-auto', className)} {...props} />
}
...
const SidebarGroupLabel: React.FC<React.ComponentProps<'div'> & { asChild?: boolean }> = ({
	...
	return (
		<Comp
			...
			className={cn(
				'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium transition-[margin,opa] duration-150 ease-linear outline-none focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const SidebarMenu: React.FC<React.ComponentProps<'ul'>> = ({ className, ...props }) => (
	<ul data-sidebar='menu' className={cn('flex w-full min-w-0 flex-col items-stretch gap-1', className)} {...props} />
)
...
const sidebarMenuButtonVariants = cva(
	'peer/menu-button flex-nowrap flex-row flex items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-[34px]! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
	{
		variants: {
			variant: {
...
> = ({ className, asChild = false, showOnHover = false, ...props }) => {
	...
	return (
		<Comp
			...
			className={cn(
				'text-sidebar-foreground ring-sidebar-ring peer-hover/menu-button:text-sidebar-accent-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 transition-transform outline-none focus-visible:ring-2 [&>svg]:shrink-0',
				// Increases the hit area of the button on mobile.
				'after:absolute after:-inset-2 after:md:hidden',
...
const SidebarMenuSubItem: React.FC<React.ComponentProps<'li'>> = (props) => <li {...props} />
