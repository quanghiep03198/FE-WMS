...
const parseColor = (input: ColorPickerProps['value'], fallback: string) => {
	try {
		if (Array.isArray(input)) return Color.rgb(input as any)
...
export const ColorPicker = ({
	value,
	defaultValue = window.matchMedia('(prefers-color-scheme:dark)').matches ? 'hsl(0 0% 98%)' : 'hsl(0 0% 3.9%)',
	onChange,
	className,
	...props
}: ColorPickerProps) => {
	const selectedColor = parseColor(value, defaultValue as string)
	...
	useEffect(() => {
		if (value) {
			const color = parseColor(value, defaultValue as string)
