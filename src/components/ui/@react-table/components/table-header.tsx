...
import {
	...
	RowData,
	type HeaderGroup,
	type ColumnFilter as TColumnFilter
...
const DataTableHeader: React.FC = memo(() => {
	const { table, event$ } = useTableContext('table', 'event$')
	const rerender = useUpdate()

	event$.useSubscription((value: { columnPinning?: ColumnPinningState; columnFilter?: TColumnFilter }) => {
...
const TableHeaderRow: React.FC<{ headerGroup: HeaderGroup<RowData> }> = memo(({ headerGroup }) => {
	return (
		<TableRow>
			{headerGroup.headers.map((header) => {
				...
				return (
					<TableHead
						...
						style={{
							height: DEFAULT_ESTIMATE_SIZE,
							width: `calc(var(--header-${header?.id}-size) * 1px)`,
							...DataTableUtility.getStickyOffsetPosition(header?.column)
						}}>
						<TableCellHead header={header} />
						{header.column.columnDef.enableResizing && <ColumnResizer header={header} />}
					</TableHead>
				)
			})}
		</TableRow>
...
const TableHeaderFilterRow: React.FC<{ headerGroup: HeaderGroup<RowData> }> = memo(({ headerGroup }) => {
