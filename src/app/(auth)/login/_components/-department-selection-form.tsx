...
const DepartmentSelectionForm: React.FC = () => {
	...
	const companyOptions = useMemo(
		() =>
			Array.isArray(data)
				? data.map((company) => ({ value: company.company_code, label: company.company_name }))
				: [],
		[data]
	);
	...
	const departmentOptions = useMemo(() => {
		const departments = Array.isArray(data)
			? data.find((company) => company.company_code === watchFields[0])?.departments ?? []
			: [];
	...
	useEffect(() => {
		if (Array.isArray(departmentOptions) && departmentOptions.length > 0)
			form.setValue('department_code', departmentOptions[0].value);
